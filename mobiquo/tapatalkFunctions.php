...
{
    global $db,$table_prefix;
	if(!tapatalk_config_table_exists())
	{
		return false;
	}
	$sql = "SELECT * FROM " . $table_prefix . "tapatalk_config WHERE config_key = '".$key."'";
    $result = $db->sql_query($sql);
    $row = $db->sql_fetchrow($result);
    if(empty($row))
    {
...
function setTapatalkConfigValue($key, $value)
{
    ...
    if(empty($row))
    {
        $sql = "INSERT INTO " . $table_prefix . "tapatalk_config (config_key,config_value) VALUES ('".$key."','".$value."')";
        $result = $db->sql_query($sql);
    }
    else
    {
        $sql = "UPDATE " . $table_prefix . "tapatalk_config SET config_value = '".$value."' WHERE config_key = '".$key."'";
        $result = $db->sql_query($sql);
    }
}
*/
